...
public:
	DES();
	void Init();
	char* EnCode(char InputKey[9]);
char* DeCode(char InputKey[9],char x);
char* DeCode(char InputKey[9], char x)
{
	for (i = 0; i < 208; i++)
		H[i] = 0;
...
private:
	...
	char InputKey[9] = "asdfghjk"; //存放字符型的八位密钥
...
char* DES::EnCode(char InputKey[9])
{
	strcpy_s(this->InputKey, InputKey);
	//CBC模式下的加密
